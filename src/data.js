...
export function calcHealth(laborRatio, bench) {
  ...
  const min = range.min;
  const max = range.max;
  if (r < min * 0.8) return { text: '⚠️ 偏低', color: '#f59e0b', bg: '#fffbeb' };
  if (r <= max) return { text: '✅ 很健康', color: '#10b981', bg: '#f0fdf4' };
  if (r <= max * 1.3) return { text: '⚡ 有問題', color: '#f97316', bg: '#fff7ed' };
